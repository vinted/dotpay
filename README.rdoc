...
Start by configuring DotpayPl:

  DotpayPl.configure do |config|
...
Sample URLC callback handling action.
  ...
  def callback
    response = DotpayPl::Response.new(params)
...
You can cancel existing transaction:
  ...
  DotpayPl.cancel_transaction(t_id, amount, control)

Also you can do partial refund:

  DotpayPl.cancel_transaction(t_id, amount, control, type: 2)
